...
function classifyAttack(payload, path, method) {
  ...
  for (const pat of sqliPatterns) {
    if (p.includes(pat.toLowerCase())) {
      let sub = "generic";
      if (p.includes("union select") || p.includes("union%20select"))
        sub = "union_based";
      else if (p.includes("or 1=1") || p.includes("or '1'='1"))
        sub = "auth_bypass";
      else if (p.includes("sleep(")) sub = "blind";

      let severity = 8; // MEDIUM-HIGH
      if (sub === "union_based" || sub === "blind")
        severity = 9; // CRITICAL
  ...
  setImmediate(async () => {
    try {
      ...
      await pool
        ...
        .catch((err) =>
          console.error("[Logger] attack_logs insert:", err.message),
        );
      ...
      await pool
        ...
        .catch((err) =>
          console.error("[Logger] attacker_profiles upsert:", err.message),
        );

      // 3. Insert into session_recordings
      ...
      if (os || tool) {
        fingerprint.updateProfile(sourceIp, os, tool).catch(() => {});
