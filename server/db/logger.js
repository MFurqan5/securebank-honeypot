...
async function logAttack(req, { attackType, severity, payload, responseCode }) {
  try {
    ...
    if (
      ...
      const idx =
        Math.abs(
          ip.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0),
        ) % mockGeoIPLocations.length;
    ...
    if (profileRes.rows.length === 0) {
      ...
      let brute =
        profile.bruteforce_count + (attackType === "bruteforce" ? 1 : 0);
      let traversal =
        profile.traversal_count + (attackType === "traversal" ? 1 : 0);
    ...
    console.log(
      `🛡️ Attack logged: [${attackType.toUpperCase()}] from ${ip} - Score updated`,
    );
...
async function logSessionAction(ip, sessionId, action) {
  try {
    const seqNum = Math.floor(Math.random() * 10000) + 1; // Simple sequence for now
