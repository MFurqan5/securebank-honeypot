...
router.get('/stats', async (req, res) => {
  try {
    ...
    // Total attacks count
    const totalResult = await pool.query('SELECT COUNT(*) as total FROM attack_logs')
...
router.get('/report/:ip', async (req, res) => {
  ...
  try {
    ...
    if (threatScore > 75) {
      riskLevel = 'CRITICAL'
      riskSummary = 'High frequency exploits detected, including SQL Injection or Path Traversal. Attempted systems takeover.'
      actions = [
      ...
      riskLevel = 'HIGH'
      riskSummary = 'Multiple vulnerability scans (SQLi, XSS) targeting application portals.'
      actions = [
      ...
      riskLevel = 'MEDIUM'
      riskSummary = 'Failed authentication attempts and probe scanning detected.'
      actions = [
    ...
    const iocList = {
      ...
      detected_scanners: profile.tool !== 'Manual (Browser)' ? [profile.tool] : [],
      attack_signatures: [
